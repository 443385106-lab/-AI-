...
ipcMain.handle('ai-generate',async(_,arg)=>{
  ...
  const requested=(arg.policyNames||arg.types||[]).filter(Boolean).slice(0,20),count=Math.min(20,Math.max(1,arg.count||requested.length||1));
  const nameList=requested.length?requested.map((x,i)=>`${i+1}. ${x}`).join('；'):'1. 管理制度';
  const prompt=`你是服务图文广告公司的中国企事业单位制度展板内容编辑与排版助手。客户提供的制度名称清单：${nameList}。${arg.autoMode?'请自动判断每张展板的适用行业、使用场景和常见规格。':`行业模板：${arg.industry}。`}共生成${count}张；有多个名称时必须按清单顺序每个名称至少生成一张，标题严格采用对应名称，不得合并或遗漏；需要补足张数时再围绕清单生成合理的配套制度。每张约${arg.words||450}字；落款：${arg.company||'未指定'}；补充要求：${arg.requirements||'无'}。正文整理为6—10条适合上墙展示的简洁条款，内容顺序为职责与适用范围、执行要求、检查记录、异常与应急、复核修订，使用中文序号和换行，不要Markdown。theme应按行业视觉习惯自动选择；recommended_width_cm和recommended_height_cm应选择图文店常用成品尺寸。不要虚构证照、法规条文编号或具体责任人；涉及法规、医疗、消防、危化、食品等内容时在review_note明确提醒交付前人工复核。`;
