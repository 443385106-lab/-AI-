...
> 仓库已新增独立的 [`native/`](native/) 工程：**匠心矢量设计 1.8 Native**。它采用 C++20 与 Qt 6 自主开发，兼容专业图文设计工作流程，但不复制任何 CorelDRAW 专有代码、界面素材或 CDR 文件规范。1.8 已加入表格线与单元格矢量重建、单栏/双栏/三栏判断和OCR阅读顺序整理；现有 Electron 制度展板工具继续独立维护。
