...
> 仓库已新增独立的 [`native/`](native/) 工程：**匠心矢量设计 1.5 Native**。它采用 C++20 与 Qt 6 自主开发，兼容专业图文设计工作流程，但不复制任何 CorelDRAW 专有代码、界面素材或 CDR 文件规范。1.5 已加入只输入制度名称自动成稿、六套行业版式、八种专业尺寸、本地制度内容和最多20张独立批量生成；现有 Electron 制度展板工具继续独立维护。
