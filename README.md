...
> 仓库已新增独立的 [`native/`](native/) 工程：**匠心矢量设计 1.6 Native**。它采用 C++20 与 Qt 6 自主开发，兼容专业图文设计工作流程，但不复制任何 CorelDRAW 专有代码、界面素材或 CDR 文件规范。1.6 已加入样图颜色与结构分析、可编辑仿制版式，以及本地模板保存、收藏、搜索、行业分类和重复应用；现有 Electron 制度展板工具继续独立维护。
