...
> 仓库已新增独立的 [`native/`](native/) 工程：**匠心矢量设计 1.7 Native**。它采用 C++20 与 Qt 6 自主开发，兼容专业图文设计工作流程，但不复制任何 CorelDRAW 专有代码、界面素材或 CDR 文件规范。1.7 已加入 Windows 10/11 本地 OCR，可提取中文、英文和数字，并支持标准制度牌重排或按样图文字区域重建可编辑对象；现有 Electron 制度展板工具继续独立维护。
